...
1. A lambda's parameters and locals live in the **enclosing method's scope**, not a fresh one — unlike an anonymous inner class body, which does open a new scope and may shadow freely. In the second example, `int x = 0, y = 0;` are already declared in `main`, so `x -> { double y = 2.0; ... }` produces two compile errors: `x` already defined (it is the lambda's own parameter name clashing with the outer `x`), and `y` already defined (the lambda's local `y` clashing with the outer `y`).
2. A captured **local** lives on the method's stack frame, which disappears when the method returns; since the lambda may outlive the call, the compiler gives it a **copy** of the value taken at creation time, and that copy is only safe if the value can never change afterward — hence *final or effectively final*. A **static field** lives in the class, not in any stack frame, so it outlives every method call; there is no lifetime mismatch to guard against, so `var = 3` is allowed to compile in `SVCExample`.
3. A **functional interface** is a Java interface with exactly one **abstract** method. Exactly one is required so that a lambda has an unambiguous method to implement and be invoked through — the arguments/result of the lambda are matched against, and it is called through, that single abstract method. Default methods carry their own implementation (they are "defined in terms of other possibly abstract methods"), so they do not count toward the abstract-method total — this is why `Iterable` can gain the default `forEach` and `Comparator` can carry many defaults while both remain valid functional-interface targets.
4. `x -> System.out.println(x)` is target-typed to `Consumer<Integer>` (its abstract method is `accept`). `intSeq.forEach(...)` calls the default method `forEach(Consumer<? super T> action)` on `Iterable`, whose body is `for (T t : this) action.accept(t)`. Each call to `action.accept(t)` runs the lambda body with `t` bound to `x`, i.e. `System.out.println(t)`. Equivalent loop:
   ```java
   List<Integer> intSeq = Arrays.asList(1,2,3);
   for (Integer t : intSeq)
      System.out.println(t);
   ```
5. `forEach` is added to `Iterable` as a **default** method, which carries an implementation. Existing classes implementing `Iterable` therefore inherit that implementation automatically and continue to compile without change — had `forEach` been added as an ordinary abstract method, every existing implementor would have failed to compile until it supplied one.
6. `x::toString` is a **specific object instance** reference: the receiver is fixed to `x`, so it satisfies `Supplier<String>` (`get()` calls `x.toString()`). `Object::toString` is an **arbitrary object of a given type** reference: the receiver becomes the method's first parameter, so it satisfies `Function<Object,String>` (`apply(o)` calls `o.toString()`), not `Supplier<String>`.
7. **No storage** — a stream conveys elements from a source rather than storing them, so the pipeline itself holds nothing and is safe to build without duplicating data. **Functional in nature** — an operation does not modify its source, so the same source can be streamed again (once a *new* stream is made) without corruption. **Laziness-seeking** — intermediate operations are always lazy, so nothing runs until a terminal operation pulls, letting the library fuse stages and skip unneeded work. **Possibly unbounded** — streams need not be finite, so short-circuiting operations like `limit(n)` or `findFirst()` let computations on infinite streams (e.g. `Stream.iterate`) finish in finite time. **Consumable** — elements are visited only once; a consumed stream cannot be reused, a fresh stream must be generated to revisit the source.
8. Output:
   ```
   Filtered value: 3
   Mapped value: 9
   ...
   Mapped value: 16
   ```
   `1` and `2` never appear (they fail `filter`); the two "Filtered" lines do not print together before any "Mapped" line. This shows laziness is per-element, not per-stage: each element is pushed through the *entire* pipeline (filter, peek, map, peek) before the next element starts, because nothing runs until `sum()` pulls one element at a time.
9. `String::concat` cannot mutate its argument, so `.reduce("", String::concat)` allocates a new string at every step, giving quadratic total work. `collect`'s three functions replace that with mutation of one container: a **supplier** constructs the result container, an **accumulator** incorporates one input element into it in place, and a **combiner** merges the contents of one container into another. The combiner is only invoked when running in **parallel** (§11.7), where separate threads build separate containers that must be merged; in a serial run it is never called.
10. `"Hello world!".chars()` returns an `IntStream` — there is no `CharStream`, since streams exist only for reference types plus `int`, `long`, `double` (§11.5, "Types of streams"). Each element is therefore an `int` character code, so `System.out::print` resolves to the `int` overload and prints the codes (`72 101 108 ...` concatenated), not the characters. Fixed by casting: `x -> System.out.print((char) x)`.
11. Soundness comes from the pipeline being **functional in nature** (pure, no side effects): if the stages don't affect anything but their own output, the order elements are processed in doesn't affect the result, so the runtime is free to split work across threads transparently — only `stream()` need become `parallelStream()`. The guarantee fails once operations **have side effects**: the `forEach` example prints `1 2 3 4 5 6 7 8` in order serially but "may print" `3 4 1 6 2 5 7 8` in parallel — order becomes unspecified, and thread safety is then the programmer's responsibility.
12. `trySplit` divides the remaining elements into two independently traversable spliterators; `Iterator` only exposes sequential `next`/`hasNext` and cannot be divided. It is needed because parallel execution requires dividing the input across multiple threads — recursively applying `trySplit` is how the runtime produces however many independent chunks of work it needs.
13. It violates the **"does not modify its source"** (non-interference) property from §11.5/§11.8: the `peek` lambda mutates `listOfStrings`, the very list backing the stream. The chapter stresses this happens "even if in single thread" — it is not a concurrency bug but a violation of the functional-in-nature contract, which the stream detects and reports by throwing.
14. `Optional.of` / `Stream.of` play the role of `return :: a -> m a`; `Optional.flatMap` / `Stream.flatMap` play the role of `(>>=) :: m a -> (a -> m b) -> m b`. `Optional` is Java's `Maybe` monad, `Stream` is Java's list monad — `Optional.flatMap` propagates emptiness as `Maybe`'s bind propagates `Nothing`, and `Stream.flatMap` replaces each element with the contents of a mapped stream, i.e. `concat(map f xs)`. What Java has is the per-type operations (`of`, `flatMap`); what it lacks is the **abstraction** — there is no `Monad` interface, so `flatMap` is duplicated on each type rather than being one class operation, and no generic `sequence` or `for` can be written once over all monads.
