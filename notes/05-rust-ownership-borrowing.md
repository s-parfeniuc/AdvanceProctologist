...
4. In the worked lifetime example of §5.7, explain precisely why uncommenting
...
1. **[O1]** every value is owned by a variable (a name, possibly a path); **[O2]** each value has at most one owner at a time; **[O3]** when the owner goes out of scope, the value is reclaimed. RAII has two defining properties: the object is bound to the scope where it is declared, and each resource has a unique owner. `[O3]` *is* the scope-binding property — reclamation happens deterministically when the scope closes. `[O2]` *is* the unique-owner property. `[O1]` is the precondition both depend on: since every value must have an owning name, there is always something for `[O2]`/`[O3]` to talk about — no ownerless value can silently escape RAII's bookkeeping.

2. `x: i32` is a `Copy` type, so `let _y = x` has copy semantics: *a new value* is created for `_y`, distinct from the one still owned by `x`. `[O2]` is satisfied trivially — two values, one owner each — so reading `x` afterwards is fine. `x: Box<i32>` does not implement `Copy`, so the same statement has move semantics: ownership of the *one* heap value moves from `x` to `_y`, and by `[O2]` it now has a single owner, `_y`. `x` is no longer a valid owner, so `println!("{}", x)` is rejected at compile time.

3. **[B1]** at most one mutable reference to a resource can exist at any time; **[B2]** if there is a mutable reference, no immutable references can exist; **[B3]** if there is no mutable reference, several immutable references can exist; **[B4]** the owner cannot free or mutate its resource while it is immutably borrowed; **[B5]** the owner cannot even read its resource while it is mutably borrowed. All five implement one principle: **aliasing and mutability cannot coexist** — either one writer and no readers, or many readers and no writer. `[B1]`–`[B3]` only constrain references among themselves; without `[B4]`/`[B5]` the *owner* would be a loophole around that rule, since it is not itself a "reference." An owner that could still mutate while immutable references exist would give readers an unstable value (breaking `[B3]`'s guarantee); an owner that could still read while a mutable reference exists would give a reader concurrent with the sole writer (breaking `[B1]`/`[B2]`'s guarantee). `[B4]` and `[B5]` extend the aliasing-xor-mutability rule to cover the owner as a participant.

4. At the point of the commented-out `println!("s-1 == {}", s)`, the mutable borrow `t` is still live: a borrow's lifetime ends at its *last use* (§5.7), and `t` is used again on the next line, `println!("t == {}", t)`. So uncommenting the `s-1` line would read the owner `s` while a mutable borrow is still in scope — a direct violation of `[B5]` (owner cannot even read while mutably borrowed). By the time `println!("s-2 == {}", s)` runs, `t`'s last use (the `println!("t == {}", t)` line) has already passed, so the mutable borrow's lifetime has ended and `s` can be read freely — permitted, since no borrow is outstanding.

5. `fn longest(s1: &str, s2: &str) -> &str` fails because there are two input lifetimes, and by `[R1]` they are distinct by default, so `[R2]` (a single input lifetime propagates to the output) does not apply — there isn't a single input lifetime. `[R3]` does not apply either, since neither parameter is `&self`/`&mut self`. With no rule assigning an output lifetime, and the true answer depending on runtime data (whichever string is actually longer), the compiler cannot pick one input's lifetime for the output. The fix names a lifetime explicitly: `fn longest<'a>(s1: &'a str, s2: &'a str) -> &'a str { if s1.len() > s2.len() { s1 } else { s2 } }`, constraining both inputs and the output to the same `'a`.

6. In `enum Tree<T> { Empty, Node(T, Tree<T>, Tree<T>) }`, a `Node` contains two `Tree<T>`s *inline*, each of which might itself be a `Node` containing two more, and so on — so its size must satisfy `size(Tree) = size(T) + 2·size(Tree)`, which has no finite solution. `Box<Tree<T>>` replaces the inline recursive fields with a pointer of fixed, known size; the recursion then goes through the heap rather than being unfolded in the struct's layout, so `size(Tree) = size(T) + 2·size(pointer)` is finite.

7. `Copy` duplicates a value by copying only the bits on the stack, with no arbitrary code run — for a field of type `Box<i32>` those bits are just the pointer, not the heap allocation it points to. If such a struct were `Copy`, then after `let y = x;` both `x` and `y` would hold a `Box<i32>` pointing at the *same* heap allocation — two owners of one resource, which `[O2]` (each value has at most one owner at a time) forbids. So `Copy` cannot be permitted whenever bitwise-copying a field would produce a second owner of a resource — i.e. whenever some field is not itself `Copy`. This is why "a struct can implement `Copy` only if all fields do" (§5.8) is really `[O2]` propagating through composite types, not an arbitrary restriction.

8. `Rc<RefCell<T>>`: `Rc<T>` contributes shared, *immutable*, reference-counted co-ownership (single-threaded, not `Send`/`Sync`); `RefCell<T>` contributes interior mutability on top, with `[B1]`–`[B3]` checked at runtime via `borrow()`/`borrow_mut()`, panicking on violation. Appropriate for shared mutable state within one thread. `Arc<Mutex<T>>`: `Arc<T>` is the thread-safe version of `Rc` — shared, immutable, atomic-refcounted ownership that is `Send`+`Sync`; `Mutex<T>` contributes mutually exclusive, checked mutation and is itself `Send`+`Sync`. Appropriate — indeed, per §5.11, the *only* solution — for shared mutable state across threads. In both pairs the outer type supplies sharing, the inner type supplies (checked) mutation; the difference is thread safety.

9. Without `Arc` (plain closure capturing `counter` by reference): rejected with `error[E0373]`, "closure may outlive the current function, but it borrows `counter`" — a spawned thread may outlive the function that owns the borrowed value, so the lifetime containment of §5.7 (owner's lifetime must exceed the borrower's) fails. The suggested `move` does not rescue it either: `move` gives the closure *ownership* of `counter`, and by `[O2]` only one closure can own it, so calling `thread::spawn(task)` a second time would fail because `task` was already consumed by the first spawn. With `Arc` (`Arc::clone` handles passed into `move` closures): sharing is now fine, but `Arc` gives only **immutable** shared access (like `Rc`, per the §5.9 table); `*c1 += 1` needs `DerefMut`, which `Arc<i32>` does not implement, giving `error[E0594]: cannot assign to data in an Arc`. The two rejections are distinct: the first is a lifetime/ownership problem (a borrowed, non-`move`d value crossing threads, or a `move`d value consumed twice); the second is a mutability problem (shared but not mutable access).

10. Inside `unsafe {}` five extra operations become possible: dereferencing a raw pointer, calling an unsafe function or method, accessing/modifying a mutable static variable, implementing an unsafe trait, and accessing fields of unions — everything else, including all of `[O1]`–`[O3]` and `[B1]`–`[B5]`, is still checked exactly as in safe code. For the doubly linked list of §5.12, this means the non-owning direction (`next: *mut Node`) can only be dereferenced inside `unsafe` blocks, and that unsafe reasoning should be confined to the data structure's own implementation, which then exposes a safe interface to its callers. This is exactly the discipline RustBelt's step 3 makes rigorous: proving that a library using `unsafe` is semantically safe *when used through its interface*, so that client code outside the data structure remains fully covered by the borrow checker.
